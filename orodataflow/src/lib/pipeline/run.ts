...
async function failRun(runId: string, error: string) {
  ...
  if (adminChat) {
    try {
      await sendTelegramMessage(adminChat, `DataFlow pipeline failed:\n${error}`);
    } catch {}
  }
}
...
export async function runPipeline(triggeredBy: "cron" | "manual"): Promise<string> {
  ...
  try {
    ...
    if (existing) {
      ...
      for (let i = 0; i < trips.length; i += BATCH_SIZE) {
        ...
        await prisma.tripRecord.createMany({
          data: batch.map((t) => ({ ...t, csvBackupId: backupId })),
        });
      }
    ...
    if (recipients.length > 0) {
      ...
      const chatIds = recipients.map((r) => r.chatId);
      const results = await sendTelegramFile(excelPath, caption, chatIds);

      for (const r of results) {
        if (r.success) sentToTelegram.push(r.chatId);
      }
    ...
    await prisma.runLog.update({
      where: { id: runId },
      data: {
        status: sentToTelegram.length < recipients.length && recipients.length > 0
          ? "partial"
          : "success",
        sentToTelegram,
        finishedAt: new Date(),
