...
interface EmailRecipient { id: string; name: string; email: string; active: boolean }
...
export default function EmailPage() {
  ...
  async function load() {
    const r = await fetch("/api/recipients/email");
  ...
  async function add(e: React.FormEvent) {
    ...
    setError("");
    setAdding(true);
    const res = await fetch("/api/recipients/email", {
      method: "POST",
  ...
  async function remove(id: string) {
    await fetch(`/api/recipients/email/${id}`, { method: "DELETE" });
    load();
  }

  async function toggle(id: string, active: boolean) {
    await fetch(`/api/recipients/email/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function test() {
    setTestMsg("");
    const res = await fetch("/api/recipients/email/test", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: testEmail }),
    });
    const d = await res.json();
    setTestMsg(res.ok ? "Test email sent!" : `Failed: ${d.error}`);
  }

  return (
    <div className="max-w-lg">
      <h1 className="text-2xl font-bold mb-1" style={{ color: "var(--text)" }}>Email Recipients</h1>
      <p className="text-sm mb-8" style={{ color: "var(--text-muted)" }}>Manage who receives the weekly summary email</p>
      ...
      <div className="rounded-xl p-5 border" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
        <h2 className="font-semibold text-sm mb-4" style={{ color: "var(--text)" }}>Test Email</h2>
        <div className="flex gap-2">
          <input type="email" value={testEmail} onChange={(e) => setTestEmail(e.target.value)} placeholder="test@example.com"
            className="flex-1 px-3 py-2 rounded-lg border text-sm outline-none"
            style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--text)" }} />
          <button onClick={test} disabled={!testEmail} className="px-4 py-2 rounded-lg text-sm font-medium border disabled:opacity-40"
